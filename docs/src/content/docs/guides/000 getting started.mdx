...
2. Install kibu's base tool chain
    ...
    See the [Toolchain Reference](/reference/toolchain/toolchain) for details on each tool.
...
```shell
kibugen_ts -output ./frontend/src/api ./...
```

This creates TypeScript types and service clients from your Go service definitions.
See the [kibugen_ts Reference](/reference/toolchain/kibugen-ts) for more details.
...
See the full [Decorators Reference](/reference/decorators) for all options.
