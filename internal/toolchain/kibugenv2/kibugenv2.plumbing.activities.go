...
func buildActivityImplementations(file *jen.File, pkg *modspecv2.Package, resolver *importResolver) {
	for _, svc := range pkg.Services {
		...
		for _, op := range svc.Operations {
			file.Add(buildActivityProxyMethod(resolver, svc, op))
			file.Add(buildActivityProxyAsyncMethod(resolver, svc, op))
		}
	}
}

func buildActivityProxyMethod(resolver *importResolver, svc *modspecv2.Service, op *modspecv2.Operation) jen.Code {
	return jen.Func().Params(jen.Id("a").Op("*").Id(firstToLower(suffixProxy(svc.Name)))).Id(op.Name).
		...
		ParamsFunc(func(g *jen.Group) {
			g.Id("res").Add(resolver.paramToExp(paramAtIndex(op.Results, 0)))
			g.Id("err").Error()
		}).
		Block(
...
func buildActivityProxyAsyncMethod(resolver *importResolver, svc *modspecv2.Service, op *modspecv2.Operation) jen.Code {
	return jen.Func().Params(jen.Id("a").Op("*").Id(firstToLower(suffixProxy(svc.Name)))).Id(suffixAsync(op.Name)).
		...
		Params(jen.Qual(kibuTemporalImportName, "Future").Types(resolver.paramToExp(paramAtIndex(op.Results, 0)))).
		Block(
			...
			jen.Return(jen.Qual(kibuTemporalImportName, "NewFuture").Types(resolver.paramToExp(paramAtIndex(op.Results, 0))).Call(jen.Id("future"))),
		)
}
